...
(function () {
  ...
  async function sendMagicLink() { const email=emailInput.value.trim().toLowerCase(); if (!client || !email) { status.textContent = window.signalboardI18n?.locale === "en" ? "Enter your email address first." : "Bitte zuerst deine E-Mail-Adresse eingeben."; emailInput.focus(); return; } if (!emailInput.checkValidity()) { status.textContent = window.signalboardI18n?.locale === "en" ? "Please enter a valid email address." : "Bitte gib eine gültige E-Mail-Adresse ein."; emailInput.focus(); return; } status.textContent = "Magic Link wird gesendet …"; const { error } = await client.auth.signInWithOtp({ email, options: { emailRedirectTo: window.location.origin } }); status.textContent = error ? error.message : "Geschickt. Öffne den Link in deiner E-Mail und lege danach ein Passwort fest."; }
  ...
  document.querySelector("#authClose").onclick = () => authDialog.close();
