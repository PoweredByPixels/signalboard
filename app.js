...
function openTinder(){const cards=jobs.filter(job=>job.status==="inbox");let index=0;const dialog=document.querySelector("#tinderDialog"),content=document.querySelector("#tinderContent");const draw=()=>{const job=cards[index];if(!job){content.innerHTML="<h2>Inbox ist leer.</h2>";return;}content.innerHTML=`<p class="eyebrow">INBOX REVIEW · ${index+1}/${cards.length}</p><h2>${job.company}</h2><p class="detail-role">${job.role} · ${job.location||"Standort offen"}</p><section><p>${job.note}</p></section><div class="modal-actions"><button class="secondary" id="tinderNo">← Archiv</button><button class="primary" id="tinderYes">Weiter →</button></div>`;document.querySelector("#tinderNo").onclick=()=>{job.status="archive";save();render();index++;draw()};document.querySelector("#tinderYes").onclick=()=>{job.status="qualified";job.isNew=false;save();render();index++;draw()}};draw();dialog.showModal();}
